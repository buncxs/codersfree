...
function discount($item)
{
  ...
  $tempQty = qty_available($item->id, $item->options->color_id, $item->options->size_id);
  if ($item->options->size_id) {
    //eliminar registro de la tabla pivote y luego volverlo a agregar
    $size = Size::find($item->options->size_id);
    $size->colors()->detach($item->options->color_id);
    $size->colors()->attach([
      $item->options->color_id => ['quantity' => $tempQty],
    ]);
...
{
  $product = Product::find($item->id);
  $quantity = quantity($product->id, $item->options->color_id, $item->options->size_id) + $item->qty;
